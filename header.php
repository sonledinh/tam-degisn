<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Tam degisn</title>
    <!--link css-->
    <link rel="stylesheet" type="text/css" title="" href="css/bootstrap.min.css">
    <link rel="stylesheet" type="text/css" title="" href="css/font-awesome.css">
    <link rel="stylesheet" type="text/css" title="" href="css/slick.min.css">
    <link rel="stylesheet" type="text/css" title="" href="css/slick-theme.min.css">
    <link rel="stylesheet" type="text/css" title="" href="css/jquery.mmenu.all.css"> 
    <link rel="stylesheet" type="text/css" title="" href="css/swiper.min.css">
    <link rel="stylesheet" type="text/css" title="" href="css/style.css">
    <link rel="stylesheet" type="text/css" title="" href="css/responsive.css">

    <script type="text/javascript" src="js/jquery.min.js"></script>
</head>
<body> 
    <header>
        <div class="header-top text-center"> 
            <div class="container">
                <h2 class="text-uppercase">TAM DESIGN - NỘI THẤT THIẾT KẾ CAO CẤP VIỆT NAM</h2>
            </div>
        </div>
        <div class="header-menu">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-md-9">
                        <div class="left">
                            <ul>
                                <li><div class="logo"><a href=""><img src="images/logo.svg" class="img-fluid" width="164" height="64" alt=""></a></div></li>
                                <li>
                                    <a href="" class="active">Phòng Khách <i class="fa fa-caret-down"></i></a>
                                    <div class="mega-menu">
                                        <ul>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li> 
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                                <li>
                                    <a href="">Phòng Bếp <i class="fa fa-caret-down"></i></a>
                                    <div class="mega-menu">
                                        <ul>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li> 
                                            <li>
                                                <div class="item text-center">
                                                    <div class="avarta"><a href=""><img src="images/mega.jpg" class="img-fluid" alt=""></a></div>
                                                    <h3><a href="">Ghế Sofa</a></h3>
                                                </div>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                                <li><a href="">Phòng Ngủ<i class="fa fa-caret-down"></i></a></li>
                                <li><a href="">Hàng Trang Trí <i class="fa fa-caret-down"></i></a></li>
                                <li><a href="">K’light <i class="fa fa-caret-down"></i></a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="right header-right">
                            <ul class="text-right">
                                <li>
                                    <a href=""><img src="images/head-1.svg" class="img-fluid" alt=""></a>
                                    <div class="box-search-head">
                                        <input type="text" placeholder="Nhập từ khóa tìm kiếm ...">
                                        <button type="submit"><i class="fa fa-search"></i></button>
                                    </div>
                                </li>
                                <li>
                                    <a href=""><img src="images/head-2.svg" class="img-fluid" alt=""></a>
                                    <div class="hver-user">
                                        <div class="head-user">
                                            <p>Đăng nhập hoặc đăng ký để lưu <br>sản phẩm yêu thích.</p>
                                            <div class="head-lg">
                                                <label>ĐĂNG NHẬP BẰNG FACEBOOK</label>
                                                <a href=""><img src="images/fb.svg" class="img-fluid" alt=""></a>
                                            </div>
                                            <div class="head-lg">
                                                <form action="">
                                                    <label>ĐỊA CHỈ EMAIL</label>
                                                    <input type="text" placeholder="Ghi địa chỉ Email của bạn tại đây" autocomplete="new-text" />
                                                    <label for="">MẬT KHẨU</label>
                                                    <input type="password" placeholder="Mật khẩu" value="123123123123" autocomplete="new-password" />
                                                    <div class="btn-head text-center"> 
                                                        <button class="text-uppercase">Tiếp tục</button>
                                                    </div> 
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li><a href=""><img src="images/head-3.svg" class="img-fluid" alt=""></a></li>
                                <li>
                                    <a href=""><img src="images/head-4.svg" class="img-fluid" alt=""></a>
                                    <div class="hver-cart">
                                        <div class="head-cart">
                                            <div class="item align-items-center d-flex">
                                                <div class="avarta"><a href=""><img src="images/pr-slide.webp" class="img-fluid" alt=""></a></div>
                                                <div class="info">
                                                    <a href="">Đèn bàn màu đen Leap</a>
                                                    <p>Số lượng: 1</p>
                                                </div>
                                            </div>
                                            <div class="item align-items-center d-flex">
                                                <div class="avarta"><a href=""><img src="images/pr-slide.webp" class="img-fluid" alt=""></a></div>
                                                <div class="info">
                                                    <a href="">Đèn bàn màu đen Leap</a>
                                                    <p>Số lượng: 1</p>
                                                </div>
                                            </div>
                                            <div class="item align-items-center d-flex">
                                                <div class="avarta"><a href=""><img src="images/pr-slide.webp" class="img-fluid" alt=""></a></div>
                                                <div class="info">
                                                    <a href="">Đèn bàn màu đen Leap</a>
                                                    <p>Số lượng: 1</p>
                                                </div>
                                            </div>
                                            <div class="item align-items-center d-flex justify-content-center" style="min-height: 60px;">
                                                <span class="text-uppercase">Và 1 mặt hàng khác</span>
                                            </div>
                                        </div>
                                        <div class="btn-cart-head text-center"><a href="" class="text-uppercase">Thủ tục thanh toán</a></div>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="menu-mobile d-none">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-md-5 col-5 col-sm-5">
                        <div class="logo"> 
                            <a title="" href="index.php"><img alt="" src="images/logo.svg" class="img-fluid avarta-logo" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-7 col-7 col-sm-7">
                        <div class="right-mobile text-right">
                            <ul class="text-right">
                                <li>
                                    <a href="javascript:void(0)"><img src="images/head-2.svg" class="img-fluid" alt=""></a>
                                    <div class="hver-user">
                                        <div class="head-user">
                                            <p>Đăng nhập hoặc đăng ký để lưu <br>sản phẩm yêu thích.</p>
                                            <div class="head-lg">
                                                <label>ĐĂNG NHẬP BẰNG FACEBOOK</label>
                                                <a href=""><img src="images/fb.svg" class="img-fluid" alt=""></a>
                                            </div>
                                            <div class="head-lg">
                                                <label>ĐỊA CHỈ EMAIL</label>
                                                <input type="text" placeholder="Ghi địa chỉ Email của bạn tại đây">
                                                <label for="">MẬT KHẨU</label>
                                                <input type="password" placeholder="*************">
                                                <div class="btn-head text-center">
                                                    <button class="text-uppercase">Tiếp tục</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li><a href=""><img src="images/head-3.svg" class="img-fluid" alt=""></a></li>
                                <li><a href=""><img src="images/head-4.svg" class="img-fluid" alt=""></a></li>
                                <li><a title="" href="#menu"><i class="fa fa-bars"></i></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <nav id="menu">
                <ul>
                    <li>
                        <input type="text" placeholder="Nhập từ khóa tìm kiếm">
                        <button><i class="fa fa-search"></i></button>
                    </li>
                    <li><a href="">Phòng Ngủ</a></li>
                    <li><a href="">Hàng Trang Trí </a></li>
                    <li><a href="">K’light </a></li>
                    <li><a href="">Phòng Ngủ</a></li>
                    <li><a href="">Hàng Trang Trí </a></li>
                    <li><a href="">K’light </a></li>
                </ul>
            </nav>
        </div>
    </header>     